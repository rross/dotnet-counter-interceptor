...
public class SimpleCounterWorkerInterceptor : IWorkerInterceptor
{
    ...
    private class WorkflowInbound : WorkflowInboundInterceptor
    {
        ...
        {
            WorkerCounter.Add(Workflow.Info.WorkflowId, WorkerCounter.NUM_OF_WORKFLOW_EXECUTIONS);
            return base.ExecuteWorkflowAsync(input);
        }

        public override Task HandleSignalAsync(HandleSignalInput input)
        {
            WorkerCounter.Add(Workflow.Info.WorkflowId, WorkerCounter.NUM_OF_SIGNALS);
            return base.HandleSignalAsync(input);
        }

        public override object? HandleQuery(HandleQueryInput input)
        {
            WorkerCounter.Add(Workflow.Info.WorkflowId, WorkerCounter.NUM_OF_QUERIES);
    ...
    private sealed class WorkflowOutbound : WorkflowOutboundInterceptor
    {
        ...
        {
            WorkerCounter.Add(Workflow.Info.WorkflowId, WorkerCounter.NUM_OF_CHILD_WORKFLOW_EXECUTIONS);
    ...
    private sealed class ActivityInbound : ActivityInboundInterceptor
    {
        ...
        internal ActivityInbound(SimpleCounterWorkerInterceptor root, ActivityInboundInterceptor next) : base(next)
        {
            this.root = root;
        } 
        ...
        {   
            WorkerCounter.Add(ActivityExecutionContext.Current.Info.WorkflowId, WorkerCounter.NUM_OF_ACTIVITY_EXECUTIONS);
